...
## Summary
- 545 nodes · 809 edges · 50 communities (43 shown, 7 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 45 edges (avg confidence: 0.75)
- Token cost: 564,044 input · 0 output
...
4. `t03 README` - 19 edges
5. `_common.md Shared Rule Block` - 17 edges
6. `plan Pass (footing/column/beam/slab)` - 12 edges
7. `_selftest()` - 11 edges
8. `t02 Workflow (Qwen3-VL-30B-A3B vs Qwen3.6-35B-A3B A/B)` - 11 edges
9. `t03 Workflow Log` - 11 edges
10. `train_qwen3vl.py Training Script` - 10 edges

## Surprising Connections (you probably didn't know these)
- `t01 PROMPT_SHORT (inference instruction)` --conceptually_related_to--> `t01 Prompt Explained (PDF)`  [AMBIGUOUS]
  t01/ai_output_บ้าน_ใหญ่_1ชั้น_01/_prompt_short.txt → t01/t01_prompt_อธิบาย.pdf
...
- `grammar_setup()` --calls--> `generate()`  [INFERRED]
  t02/data_before_tune/run_house_batch.py → t03/data_before_tune/infer_house_t03.py
...
Cohesion: 0.08
Nodes (48): cv_hint_text(), demo(), draw_som_marks(), number_elements(), page_hint(), แปลง scan → บล็อกข้อความแปะท้าย prompt pass 2.4 (2.4a=ข้อความล้วน, 2.4b=คู่ภาพมา, เอา detection ของหน้านี้เอง (ทุกตัวที่ผ่านคลังกลาง) เป็น template กวาดซ้ำเข้ม 0., ลายนิ้วมือหยาบๆ จากจำนวนที่เจอ — ไว้เช็คขวางป้าย pass0 ไม่ใช่แทนที่มัน (+40 more)

### Community 1 - "Harvest Report - Template Match QA"
Cohesion: 0.08
Nodes (50): Bank of 10 Templates, พอดี (Exact Match) - GT bank count matches template count, เกิน (ขยะ) - Excess False Positive matches, Harvest Report, 01บ้าน_เล็ก_1ชั้น_01, 02บ้าน_เล็ก_1ชั้น_02, 03บ้าน_เล็ก_2ชั้น_01, 04บ้าน_เล็ก_2ชั้น_02 (+42 more)

### Community 2 - "T01 Batch House Extraction (GPU-Rental)"
Cohesion: 0.09
Nodes (31): Path, discover_pages(), generate(), load_model(), log(), main(), variant='tuned' -> base+adapter (PEFT); variant='base' -> base เพียวๆ ไม่มี adap, xgrammar builtin JSON grammar — เหมือน t02's run_house_batch.py (rule_of_tune.md (+23 more)
...
- **Why does `main()` connect `T03 GT-vs-AI Overlay Renderer` to `T01 Batch House Extraction (GPU-Rental)`?**
  _High betweenness centrality (0.009) - this node is a cross-community bridge._
- **Why does `_selftest()` connect `T03 GT-vs-AI Overlay Renderer` to `T01 Batch House Extraction (GPU-Rental)`?**
  _High betweenness centrality (0.009) - this node is a cross-community bridge._
- **Why does `find_image()` connect `T03 Dataset Builder (build_dataset_t03.py)` to `T01 Batch House Extraction (GPU-Rental)`?**

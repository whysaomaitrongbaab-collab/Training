...
7. **แม้ไฟล์จะเข้าเงื่อนไข "ไม่เข้าข่ายกฎข้อ 1" (เช่น เอกสาร .md อย่าง `primary_rawjson_schema.md`) ก็ต้องพูดออกมาให้ชัดว่าทำไมถึงแก้ได้เลยโดยอ้างอิงกฎข้อนี้ ก่อนหรือระหว่างแก้ ห้ามแก้เงียบๆ แล้วไม่บอกเหตุผล** — บทเรียนจาก: แก้ `primary_rawjson_schema.md` (เพิ่ม section สรุป `element_type` ของ `site_plan`) โดยไม่ได้พูดถึง `rule_of_tune.md` เลยระหว่างทำ ทั้งที่เพิ่งรับปากไปว่าจะอ้างอิงกฎนี้ทุกครั้งที่เกี่ยวกับ scope การแก้ไข ผู้ใช้ต้องเตือนเอง — ต้องอ้างอิงชื่อไฟล์/ข้อกฎเสมอเมื่อการกระทำเกี่ยวข้องกับขอบเขตการแก้ไขข้อมูลในโปรเจกต์นี้ ไม่ว่าไฟล์นั้นจะเข้าข่ายกฎข้อ 1 หรือไม่ก็ตาม **ห้ามแก้ `primary_rawjson_schema.md` โดยไม่ได้อ้างอิง `rule_of_tune.md` เด็ดขาด — กฎนี้ผูกกับไฟล์นี้โดยเฉพาะ ไม่ใช่แค่หลักการทั่วไป** **และทุกครั้งที่แก้ `primary_rawjson_schema.md` ต้องบันทึกลง [`raw_json_ตัวที่ใช้งานจริง/primary_rawjson_schema_edit_log.md`](../../raw_json_ตัวที่ใช้งานจริง/primary_rawjson_schema_edit_log.md) ทันที (เพิ่ม 2026-07-13) — คู่กับ `raw_json_data_log.md` ที่บันทึกการแก้ข้อมูล แต่ไฟล์นี้บันทึกเฉพาะการแก้ตัวสเปคเอง**

8. **อย่าสร้างของซ้ำ (memory ส่วนตัวของ AI, ไฟล์บันทึกภายนอก ฯลฯ) เพื่อจำกฎที่ควรจะอยู่ในเอกสารโปรเจกต์นี้อยู่แล้ว** — บทเรียนจาก (2026-07-13, น่าอับอาย): หลังเพิ่งเพิ่มกฎข้อ 7 เข้า `rule_of_tune.md` เอง ยังไปสร้าง memory file ส่วนตัว (`feedback_cite_rule_of_tune.md`) ซ้ำเรื่องเดียวกันอีก ทั้งที่กฎอยู่ใน `rule_of_tune.md` ซึ่งเป็น single source of truth ของโปรเจกต์นี้อยู่แล้ว — ผู้ใช้ต้องสั่งให้ลบทิ้งเอง ต่อไปนี้ถ้ากฎ/บทเรียนเกี่ยวกับโปรเจกต์นี้ควรบันทึก ให้บันทึกที่ `rule_of_tune.md` (หรือเอกสารโปรเจกต์ที่เหมาะสม) เท่านั้น ไม่ต้องสร้าง memory ส่วนตัวซ้อนอีกชั้น

9. **การ "อ้างอิงกฎ" ต้องเป็นจังหวะหยุดถามจริง ไม่ใช่แค่ท่องชื่อกฎประกอบก่อนลงมือทำต่อเอง** — บทเรียนจาก (2026-07-14, น่าอัปยศ): พูดอ้าง "ตาม rule_of_tune.md กฎข้อ 7" ก่อนแก้ `primary_rawjson_schema.md` (เพิ่ม section 10a เรื่องบันได) แต่พูดจบแล้วรีบลงมือแก้ไฟล์ทันทีในข้อความเดียวกัน ไม่ได้หยุดรอให้ผู้ใช้ตอบ yes/no จริงๆ — ผู้ใช้ต้องถามตรงๆ ว่า "ทำไมแจ้ง rule of tune แล้วไม่กดขออนุญาติ yes/no วะ" ถึงจะรู้ตัว — **ทุกครั้งที่อ้างอิง `rule_of_tune.md` เป็นเหตุผลก่อนแก้ไฟล์ (โดยเฉพาะ `primary_rawjson_schema.md` ตามกฎข้อ 7) ต้องหยุดจริง ส่งข้อความถามแล้วรอคำตอบ ห้ามอ้างแล้วแก้ต่อในข้อความเดียวกัน**

10. **ห้ามตีความข้อความสั้น/กำกวมของผู้ใช้เองว่าเป็นคำตอบ yes/no ต่อคำถามที่เคยถามค้างไว้ — ต้องย้ำคำถามให้ชัดแล้วรอคำตอบที่ชัดเจนจริงๆ ก่อนลงมือ** — บทเรียนจาก (2026-07-14, น่าอัปยศ ต่อเนื่องจากกฎข้อ 9): หลังถาม "จะเพิ่ม `specs{}` เข้า `primary_rawjson_schema.md` ไหม yes/no?" แล้วหยุดรอจริงตามกฎข้อ 9 ผู้ใช้ตอบเรื่องอื่นก่อน (สั่งให้ทำ workmen diary) จากนั้นในข้อความถัดมาพิมพ์สั้นๆ ว่า "เอ่อแก้ใน primary rawjson ด้วย" — Claude ตีความเองทันทีว่านี่คือคำตอบ "yes" ต่อคำถามเดิม แล้วลงมือแก้ `primary_rawjson_schema.md` และ log ทันทีโดยไม่ได้ย้ำถามซ้ำให้ชัดก่อน ผู้ใช้ต้องเตือนตรงๆ ว่าทำ "โดยไม่ขออนุญาต" — **แม้จะเคยถามและหยุดรอจริงมาก่อนแล้ว ถ้าคำตอบที่ได้กลับมาไม่ใช่ yes/no ตรงๆ (เป็นข้อความสั้น/กำกวม/พูดเรื่องอื่นแทรก) ต้องย้ำคำถามเดิมให้ชัดอีกครั้งก่อนลงมือ ห้ามเดาเจตนาเอาเอง**

...
- **Label Studio review flow** — wire แล้ว (ดูหัวข้อ "Label Studio Cloud — review flow" ด้านล่าง) แต่ยังไม่ publish/เชิญคนจริง และยังไม่มี pre-annotation (ไม่มี `<house>-qwen.json` ให้แนบ)
...
| `label-studio-import-annotations.js` | แปลง export JSON จาก Label Studio → `annotated/<record_id>-annotated.json` + อัปเดต `manifest.json` |
...
- รัน extraction pipeline (`run_pipeline.py`) ให้ครบทุกบ้าน แล้ว export เป็น `qwen-output/<house>-qwen.json` (รูปแบบรวมทั้งบ้าน ไม่ใช่ต่อหน้า) เพื่อให้ `label-studio-tasks-github.js` แนบเป็น pre-annotation ได้ — ตอนนี้เพื่อนจะเห็นกล่อง JSON ว่างเปล่า ต้อง label จากศูนย์ทุกบ้าน ยังไม่ได้ประโยชน์จาก pre-labeling ตามที่ตั้งใจไว้ใน `docs/FINETUNING_FLOW.md` (repo หลัก)

...
function main() {
  ...
  const subfolder = process.argv[3] || null;
  ...
  const dir = subfolder
    ? path.join(__dirname, '..', 'mk_test', subfolder)
    : findProductionDir(house) || path.join(__dirname, '..', 'mk_test', 't2');
  if (!fs.existsSync(dir)) {
    console.error(`❌ ${dir} not found`);
    process.exit(1);
  }
  console.log(`ℹ️  Reading from: ${dir}`);
  ...
  fs.writeFileSync(path.join(__dirname, 'element', elementsFile), JSON.stringify(elementsTasks, null, 2), 'utf-8');
  fs.writeFileSync(path.join(__dirname, 'material_list', materialListFile), JSON.stringify(materialListTasks, null, 2), 'utf-8');
  fs.writeFileSync(path.join(__dirname, 'single', singleFile), JSON.stringify(singleTasks, null, 2), 'utf-8');

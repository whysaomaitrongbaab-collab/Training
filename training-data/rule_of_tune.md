...
**ต้องอ่านไฟล์นี้ก่อนเริ่มงานทุกครั้ง** ในโฟลเดอร์ `training-data/` — ไม่มีข้อยกเว้น
...
ถ้าจำเป็นต้องแก้ raw JSON จริง (เช่น รัน pipeline ใหม่ทับของเดิม, แก้ค่าที่ผิดมือ) และได้รับอนุญาตแล้ว:
- สำรอง/เช็ค git status ก่อนแก้เสมอ (ให้กู้คืนได้ถ้าพลาด)
- บันทึกใน `CLAUDE.md` ว่าแก้อะไร ทำไม เมื่อไหร่

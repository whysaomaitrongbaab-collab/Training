...
- **Span:** read from the grid, never eyeballed off the drawing. **Amended 2026-08-29 (Makham's order):** when a span is printed nowhere, the model *may* recover it by proportion against two grid lines whose `pos_m` it already knows — that is measuring against a ruler printed on the same image, which is what the grid master exists for, and it replaces a `null` that used to be thrown away. It is a derived value, never a read one, so it carries its own `span_source` (below) and a lower `confidence_score`. A printed value always wins; the x-axis scale measures x spans and the y-axis scale measures y spans (a cropped/rescaled scan does not stretch both axes equally); a diagonal span stays `unresolved`. Same rule extends to a dummy grid line whose position the dimension chain never prints. Does **not** apply to a sheet with no grid to calibrate against, e.g. `section` — there, an unprinted dimension is still `null`.
- **`span_source` enum:** `grid_table` / `local_dimension` / `scaled_from_grid` / `unresolved` / `n/a`

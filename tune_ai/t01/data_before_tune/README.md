...
> ⚠️ **[rule_of_tune.md](../../../No_touch_box/docs/rule_of_tune.md) ข้อ 2 ส่วนขยาย** — ทุกไฟล์ในโฟลเดอร์นี้กำหนดว่าอะไรจะกลายเป็น "ตัวอย่าง training"
> แก้ `CFG` ใน `build_dataset.js` หรือ `CONFIG`/`PRESETS` ในสคริปต์เทรน = เปลี่ยนสิ่งที่โมเดลจะเรียน ต้องเตือน/ขออนุญาตก่อนเสมอ
> โฟลเดอร์นี้ไม่แตะ `rawjson_ยังไม่ได้แก้ไขโดนคน/` เลย (ไม่เข้ากฎข้อ 1)
...
**83% ของ target ยังไม่มีคนตรวจ** — diff เทียบ `rawjson_ยังไม่ได้แก้ไขโดนคน/` แล้วมีแค่ **80 จาก 474 ไฟล์** ที่แก้จริง อีก 394 ไฟล์เหมือน raw เป๊ะ
...
**เรื่องอื่นที่ยังค้าง:** บ้าน 05-10 ยังไม่เข้าชุด · Label Studio ยังไม่เคยรีวิวจริงสักรอบ · ยังไม่ sync กลับ `rawjson_ยังไม่ได้แก้ไขโดนคน/`

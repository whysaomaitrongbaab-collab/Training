...
# ข้อความเดียวใช้ทั้งทางเช่าเองและทางอัตโนมัติ — เคยเป็นสองก๊อปปี้ที่เหมือนกันคำต่อคำ
# ถ้าแก้ที่เดียวลืมอีกที่ จะเพี้ยนแบบไม่มีใครเห็น
READY = """
✅ พร้อมสาธิตแล้ว
...
def bring_up(instance_id=None):
    ...
    if instance_id:
        ...
        print(f"\n▶ กำลังเช่าการ์ด + เปิดโมเดล [{model}] — ใช้เวลา 20-45 นาที "
              f"(ส่วนใหญ่รอโหลดโมเดล 70GB)")
        cmd = ["up", "--model", model, "--yes", "--max-price", "1.5"]
    ...
    if run(["smoke"]).returncode != 0:
        print("\n⚠️ ทดสอบไม่ผ่าน — ยังไม่ควรไปสาธิตสด")
        print("   ⚠️ การ์ดยังเปิดอยู่และเงินเดินอยู่ — **ห้ามกดข้อ 1/3 ซ้ำ** จะเป็นการเช่าใหม่ทั้งที่ของเดิมยังดี")
        print("   โมเดล 70GB อาจยังโหลดไม่เสร็จ รอสัก 2-3 นาทีแล้วไปข้อ 9 → 1 ทดสอบซ้ำ")
        print("   พอทดสอบผ่านแล้ว ข้อ 9 → 3 เปิดตัวรับงาน = พร้อมสาธิตเหมือนเดิม")
        return
...
def do_worker():
    """เปิดหน้าต่าง worker แยก — ครึ่งหลังของข้อ 1/3 สำหรับกรณีการ์ด+โมเดลพร้อมอยู่แล้ว
    (เช่นเปิดค้างไว้สะดุดกลางทางแต่การ์ดรอด, หรือเผลอปิดหน้าต่างรับงานไป)"""
    if not current_instance():
        print("\n❌ ยังไม่มีการ์ดเปิดอยู่ — เลือกข้อ 1 หรือ 3 ก่อน")
        return
    start_worker()
    print("\n✅ เปิดหน้าต่างรับงานแล้ว — ห้ามปิดตลอดช่วงสาธิต")
...
REPAIR_MENU = """
  ...
  3  เปิดตัวรับงาน      (การ์ดเปิดอยู่แล้วแต่หน้าต่างรับงานปิดไป)
  4  ปลดงานถอดแบบที่ค้าง (เผลอปิดหน้าต่างรับงานตอนกำลังถอดแบบ แล้วงานค้างไม่มีใครทำต่อ)
  0  กลับเมนูหลัก
"""
...
def repair_tools():
    ...
    while True:
        ...
        act = tools.get(c)
        if not act:
            print("\n❌ ไม่มีข้อนี้ พิมพ์เลข 0-3 เท่านั้น")
            continue
        try:
            act()

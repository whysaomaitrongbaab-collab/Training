...
def do_start():
    ...
    if iid:
        ...
        if input("   คืนการ์ดเดิมแล้วเริ่มใหม่เลยไหม? [y/N] ").strip().lower() != "y":
            print("   ยกเลิก — ถ้าการ์ดเดิมยังใช้ได้อยู่ ให้ข้ามไปข้อ 6 (เปิดตัวรับงาน) ได้เลย")
            return
        run(["down"])
...
╚══════════════════════════════════════════════╝
  ...
  6  เปิดตัวรับงาน  (worker — ใช้เมื่อการ์ดเปิดอยู่แล้วแต่หน้าต่างรับงานยังไม่เปิด/ปิดไปแล้ว)
  0  ออก
"""


def do_worker():
    """เปิดหน้าต่าง worker แยก — ครึ่งหลังของข้อ 1 สำหรับกรณีการ์ด+โมเดลพร้อมอยู่แล้ว
    (เช่นข้อ 1 สะดุดกลางทางแต่การ์ดรอด, หรือเผลอปิดหน้าต่างรับงานไป)"""
    if not current_instance():
        print("\n❌ ยังไม่มีการ์ดเปิดอยู่ — เลือกข้อ 1 ก่อน")
        return
    subprocess.Popen(["cmd", "/c", "start", "cmd", "/k",
                      f'cd /d "{HERE}" && "{PY}" worker.py'], shell=False)

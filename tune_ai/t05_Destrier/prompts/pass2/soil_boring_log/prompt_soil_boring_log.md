...
Marked *(draft)* in `primary_rawjson_schema.md` §1 - the field set has not been verified
...
Prepend `../../_common.md`. No embedded glossary here (2026-08-29 review) - this pattern's
...
- `element_id` - the printed layer label if there is one, otherwise `layer_1`, `layer_2` and so
  on in printed order, top of hole first (§0.2 allows a descriptive id, never leave it absent)
...
- `uscs` is the printed classification symbol (`SM`, `CH`, `CL` and the like) when the log
  prints one
- Unit weight is `unit_weight_kn_m3` as a number Thai logs commonly print γt in t/m³ - if the
  sheet prints `1.80 ต/ม³`, convert (multiply by 9.81) and keep `1.80 ต/ม³` in
  `unit_weight_printed_as` Never record a t/m³ figure in a `_kn_m3` field

SPT N-values

- A plain number goes in `spt_n` as a number
- `R`, `Refusal`, or `>100` means the sampler could not advance - record `spt_n: 100`,
  ...
  Do not write `null` - refusal is a real, meaningful reading (very dense or hard stratum), and a
  `null` reads downstream as not tested
- A blow-count triple (`5-8-11`) means `spt_n` = the sum of the last two (the standard N value),

...
**Not wired into `build_dataset_t03.py` - no training examples exist.** `PLAN_SUBTASKS` has no `plan_column` entry (a real gap, see `../../pass_io_table_detailed.csv`) and no house in the corpus has a standalone column-plan sheet - columns always show up as points on a footing_plan or beam_plan sheet instead. Kept here as a rendered reference in case a future house has one.

**Input:** one `plan_column` sheet image plus the building's grid master (`x_lines`/`y_lines` only, extracted from `<house>_หน้า00_gridline.json`).
...
You are reading a structural plan sheet Extract columns (เสา)

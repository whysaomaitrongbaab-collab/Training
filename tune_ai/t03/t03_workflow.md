...
## ภาคผนวก — ตรวจกำลังรับภาพ (MAX_PIXELS / MAX_LENGTH) 2026-08-25

มะขามสั่งจากประชุมอาจารย์: "max token ตอนนี้ 768 อยากเพิ่ม" → เลข 768 ที่เห็นคือ
`video_processor.max_frames` ใน `processor_config.json` = เพดานเฟรม**วิดีโอ** ซึ่งไปป์ไลน์นี้
ไม่เคยใช้ (ส่งภาพนิ่งทุกครั้ง `{"type":"image"}`) — ปรับแล้วไม่มีผลใดๆ ปุ่มจริงคือ 2 ตัวข้างล่าง

วัดด้วย `data_before_tune/measure_capacity.py` (เขียนใหม่ 2026-08-25, อ่านอย่างเดียว ไม่ใช้ GPU)
ส่วน visual token คำนวณตรงจากขนาดไฟล์ภาพ (patch 16 × merge 2 → 1 token = 1024 px) = **แม่นยำ**
ส่วนข้อความประมาณด้วย chars/2.2 (สูตรเดียวกับ `_est()` ใน train_t03.py) = **ประมาณการ**

### ผลวัด train.jsonl (854 ตัวอย่าง / 973 ภาพ)

| ขนาดภาพ | จำนวน | token เต็ม | ที่ cap 5,120 |
|---|---|---|---|
| 3309×2339 | 615 | 7,558 | เสีย 32% |
| 3355×2347 | 176 | 7,690 | เสีย 33% |
| 4631×3473 | 116 | 15,707 | **เสีย 67%** |
| 3307×2339 | 65 | 7,554 | เสีย 32% |
| 3308×2339 | 1 | 7,556 | เสีย 32% |

**973/973 ใบ (100%) โดนย่อทั้งหมดที่ค่าปัจจุบัน** — ไม่ใช่บางใบ
⚠️ ขัดกับที่บันทึกไว้ในตารางแถว 7 ("ภาพจริง 2339×1654 ต่ำกว่า cap 5,120 อยู่แล้ว = ไม่ย่อเลย")
— ในชุดเทรนปัจจุบันไม่มีภาพขนาดนั้นเลยสักใบ ตัวเล็กสุดคือ 3307×2339 = 7,554 token
**หมายเหตุความไม่แน่นอน:** train.jsonl บนดิสก์ timestamp 22:13 แต่ TEST_STEPS รัน 22:00
= ชุดถูก rebuild หลังการทดสอบนั้น จึงยืนยันไม่ได้ว่า `train_ds[0]` ตอน 22:00 เป็นตัวเดียวกัน
→ **ต้องอ่านค่าที่ assert พิมพ์ออกมาในการรันครั้งหน้า อย่าเชื่อตัวเลข 3,778 เดิม**
**ให้ความเป็นธรรมกับบันทึกเดิม:** ภาพขนาด 2339×1654 (= 3,778 token พอดีเป๊ะ) **มีอยู่จริง 70 ใบ
แต่อยู่ใน val ไม่ใช่ train** — ตัวเลขนั้นจึงไม่ได้ถูกกุ เพียงแต่ไม่ใช่ตัวแทนของชุดเทรน
(val: 171/241 ใบโดนย่อ = 71%, ยังมีภาพเล็กปนอยู่ / train: 973/973 = 100%)

### ตัวเลือกถ้าจะเพิ่ม (ยังไม่ได้ทำ รอมะขามเคาะ — เปลี่ยนแล้วต้องเช่า GPU ใหม่ถึงรู้ผล)

| MAX_PIXELS | ภาพที่ยังโดนย่อ | seq สูงสุด (ประมาณ) | MAX_LENGTH ที่ต้องใช้ |
|---|---|---|---|
| **5,120 (ตอนนี้)** | 973/973 (100%) | 34,854 | 36,864 |
| 7,680 | 292/973 (30%) | 44,607 | 47,104 |
| 7,680 + ลดภาพ gridline 4→3 | 292/973 | 37,049 | 39,936 |
| 16,384 (ไม่ย่อเลย) | 0/973 | 71,504 | 75,776 |

ตัวที่ดันให้ seq ยาวคือ **gridline ล้วนๆ** (36 ใน 37 ตัวที่ทะลุใช้ 4 ภาพ) — subtask อื่นไม่แตะเพดาน
ดังนั้น "ลดภาพ gridline 4→3" (ซึ่ง docstring ของ train_t03.py แนะนำไว้อยู่แล้วเป็นทางแก้ OOM)
คือทางที่ได้ความละเอียดเพิ่ม 32% โดย MAX_LENGTH ขึ้นแค่ 39,936

⛔ **VRAM คือข้อจำกัดจริง ไม่ใช่โมเดล** — โมเดลรับได้ 262,144 (เราใช้ 12%) แต่รอบที่แล้ว
32,768 + r=32 OOM จริง ต้องลดเหลือ r=16 และ eval ยัง OOM จาก fp32 logits upcast
เพิ่ม MAX_LENGTH โดยไม่ลด r หรือไม่เพิ่มการ์ด = OOM ซ้ำแน่นอน

### แก้บั๊กแล้ว 1 จุดใน train_t03.py (ตัวตรวจสอบ ไม่ใช่ค่าเทรน)

assert ตัวแรกคอมเมนต์ว่า "ตัวอย่างแรก (ภาพเดียว)" แต่ `train_ds[0]` ของชุด op04 มี **4 ภาพ**
และ `_tok` ใช้ `.sum()` รวมทุกภาพแล้วเทียบเกณฑ์ `> 1000` ของภาพเดียว → เกณฑ์หลวมลง 4 เท่า
**หายนะแบบ t01 (266 token/ภาพ) × 4 ภาพ = 1,064 > 1000 → assert ผ่าน = ตัวกันภัยจับไม่ได้**
แก้เป็นหาร `n_img` ให้เป็นต่อภาพจริง + พิมพ์ cap ที่ตั้งไว้เทียบให้เห็น + เตือน (ไม่ abort)
ถ้าได้ต่ำกว่า cap เกิน 25% (กรณี collator ย่อภาพเองเพื่อให้พอดี max_seq_length)

### ✅ ตัดสินใจแล้ว 2026-08-25 — เพิ่ม MAX_PIXELS 5,120 → 7,680 (ไม่เปลี่ยนการ์ด)

มะขามสั่ง "เพิ่ม VRAM" → คำนวณก่อนแล้วพบว่า **ไม่ต้องเปลี่ยนการ์ดก็เพิ่มได้ 32%**
แยกส่วน VRAM จากจุดวัดจริงของรอบที่แล้ว (peak 87.5 GB @ seq 32,768, r=16, การ์ด usable 94.97):

| ส่วน | GB | โตตาม seq ไหม |
|---|---|---|
| น้ำหนักโมเดล bf16 | 74.0 | ไม่ |
| LoRA + grad + optimizer (r=16) | 5.7 | ไม่ |
| **activation** | **7.8** | **ใช่ (เชิงเส้น — flash attention)** |

| ตัวเลือก | MAX_LENGTH | activation | peak | การ์ดเดิม 96GB |
|---|---|---|---|---|
| 5,120 (เดิม) | 32,768 | 7.8 | 87.5 | ✅ เหลือ 7.5 |
| **7,680 ← เลือกอันนี้** | **47,104** | **11.3** | **90.9** | **✅ เหลือ 4.0** |
| 7,680 + gridline 4→3 | 39,936 | 9.5 | 89.2 | ✅ เหลือ 5.8 (แผนสำรองถ้า OOM) |
| 16,384 (ไม่ย่อเลย) | 75,776 | 18.1 | 97.8 | ❌ เกิน 2.8 → ต้อง H200 |

**แก้แล้วใน `train_t03.py`:** `MAX_PIXELS = 7680*1024`, `MAX_LENGTH = 47104` (r=16 เท่าเดิม)
ตรวจแล้ว: syntax ผ่าน · assert ตัวยาวสุดจะได้ 44,606 < 47,104 = **ผ่าน margin 5.3%**

⚠️ **margin VRAM แคบลงจาก 7.5 → 4.0 GB — ต้องรัน `TEST_STEPS=5` ยืนยันก่อนรันเต็มเสมอ**
(rule_of_tune ข้อ 4 · รอบที่แล้ว OOM ตอนเหลือ 470 MiB มาแล้ว) ถ้า OOM: ลดภาพ gridline 4→3
ก่อน (peak 89.2) แล้วค่อยถอย MAX_PIXELS — **ห้ามลด MAX_LENGTH เฉยๆ โดยไม่ลด MAX_PIXELS**

### ราคาการ์ดใหญ่ (เช็คสด 2026-08-25 ผ่าน `vastai search offers`) — ยังไม่ได้เช่า

| การ์ด | VRAM | $/ชม. | 1 รอบ (5.3 ชม.) |
|---|---|---|---|
| RTX PRO 6000 (ใช้อยู่) | 96GB | 1.107 | $5.87 |
| H200 ถูกสุด (Virginia US) | 140GB | **3.293** | **$17.45** |
| H200 NVL (Czechia) | 140GB | 3.304 | $17.51 |

⛔ **ตัวบล็อกจริง: เครดิตเหลือ $8.91 (ตัวเลขล่าสุดที่จดไว้) = เช่า H200 เทรนไม่จบ 1 รอบ**
ต้องเติมเงินก่อนถึงจะไปเส้นทาง 16,384 ได้ — ที่ 140GB จะรันได้ทั้ง **ไม่ย่อภาพเลย + คืน r=32**
(peak 103.4 GB เหลือ 36 GB) ซึ่งเป็นสองอย่างที่รอบ t03 ต้องยอมตัดทิ้งทั้งคู่

---

## ⛔ บทเรียน 2026-08-25 — "section 0%" ส่วนใหญ่ไม่ใช่โมเดลอ่านไม่ออก แต่เป็นตัววัดที่วัดสิ่งที่เป็นไปไม่ได้

มะขามถาม "section ได้ 0 เลย ใช้ pattern recognition ได้ไหม" → ไล่ดูก่อนลงมือ แล้วพบว่า
**ต้องแก้ตัววัดก่อน ไม่ใช่แก้โมเดล** วัดด้วย `data_before_tune/measure_id_ceiling.py` (ใหม่, read-only)

### รากของปัญหา

`infer_house_t03.py:218` วัดด้วย `set(gt_ids) & set(pred_ids)` = **เทียบสตริงตรงเป๊ะ ไม่ normalize**
แต่ `element_id` ใน GT มี 2 พันธุ์ปนกัน:

| พันธุ์ | ตัวอย่าง | โมเดลเดาถูกได้ไหม |
|---|---|---|
| ชื่อที่พิมพ์บนแบบ | `B1` `B3X` `C1` `F1A` `RB1'` | ✅ ได้ |
| **ชื่อที่คนจดตั้งเอง** | `gate_front` · `SP_at_bathroom` · `purlin_rafter_connection` · `door_lock_detail` · `ห้องนอน` · `ระดับพื้นอาคาร` | ❌ **ไม่มีทาง** |

### เพดานสูงสุดที่โมเดลทำได้ แม้อ่านแบบถูกทุกตัว

| subtask | id ทั้งชุด | อ่านได้จากแบบ | **เพดาน** | t03 ทำได้ | อ่านว่า |
|---|---|---|---|---|---|
| plan_beam | 2,484 | 2,175 | **88%** | 0% | 🔴 **โมเดลพังจริง** |
| plan_slab | 283 | 261 | **92%** | 20% | 🔴 โมเดลพังจริง |
| plan_footing | 462 | 345 | 75% | 33.3% | 🟡 พอมีช่อง |
| schedule | 662 | 403 | 61% | 21.9% | 🟡 พอมีช่อง |
| **section** | 2,436 | 902 | **37%** | 0% | 🟠 **ตัววัดพังครึ่งหนึ่ง** |
| notes | 272 | 5 | 2% | – | 🟠 ตัววัดพังเกือบหมด |

**เฉพาะชุด test (บ้าน 08) ที่ใช้รายงานผล เพดาน section = 1/23 = 4%**
บ้าน 08 เป็นบ้านชั้นเดียวที่หน้า section แทบทั้งหมดเป็นรายละเอียดรั้ว/ประตู/ท่อ ไม่ใช่หน้าตัดคาน-เสา
→ **แม้โมเดลสมบูรณ์แบบก็ได้ไม่เกิน 4%** เลข 0% จึงแทบไม่มีข้อมูลเรื่องความสามารถโมเดลเลย

### ข้อสรุปที่ต่างกันของ 2 ตัวเลข 0% — สำคัญ อย่าสับสน

- **plan_beam 0% = ของจริง** เพดาน 88-93% แปลว่าโมเดลอ่านไม่ออกจริง → **pattern recognition ช่วยตรงนี้ได้** (เครื่องมือ `tools/pattern_recognition.py` ที่ทำวันนี้เล็งเป้านี้)
- **section 0% = ตัววัดเป็นหลัก** ไม่มีอะไรบนกระดาษให้ตรวจจับ → pattern recognition **ช่วยไม่ได้** เพราะปัญหาไม่ได้อยู่ที่การมองเห็น

### ทดลอง pattern recognition กับ section แล้ว — ไม่ผ่าน บันทึกไว้กันทำซ้ำ

| วิธี | บ้าน 17 | บ้าน 12 | บ้าน 01 |
|---|---|---|---|
| HoughCircles หาวงกลม mark (B1)(B2) | **5/5 ✅** | 3/20 ❌ | 0/18 ❌ |

สาเหตุ: **หน้า section ของแต่ละสำนักออกแบบคนละ layout** — บ้าน 17 ใช้วงกลมล้อมชื่อ,
บ้าน 01 ใช้ข้อความขีดเส้นใต้ "B1 1:20" ไม่มีวงกลมเลย, และหน้าตัดที่มีเส้น grid พาดผ่าน
ทำให้ contour ไม่เป็นสี่เหลี่ยม 4 มุม (auto-detect กล่องได้ 7 จาก 14)
ต่างจากผังโครงสร้างที่สัญลักษณ์นิ่งกว่ามาก

### ทางแก้ที่เสนอ (ยังไม่ได้ทำ — แตะ GT = Rule 1/2 ต้องขออนุญาต)

1. **แยกตัววัด** — รายงาน id-recall เฉพาะ id ที่พิมพ์บนแบบ และรายงาน id ที่คนตั้งเองแยกอีกคอลัมน์
   (แก้ที่สคริปต์วัด ไม่แตะ GT = ปลอดภัยสุด ทำได้เลยถ้ามะขามสั่ง)
2. **normalize ก่อนเทียบ** — strip/uppercase อย่างน้อย กัน `"B1 "` ไม่ตรง `"B1"`
3. เปลี่ยน convention การตั้ง `element_id` ของ section (แตะ GT — งานใหญ่ ต้องขออนุญาต)

**⚠️ ข้อจำกัดของการวัดนี้:** `PRINTED_ID` เป็น regex heuristic — คำอย่าง `ห้องนอน` ถูกจัดเป็น
"คนตั้งเอง" ทั้งที่พิมพ์อยู่บนแบบจริง (แต่เป็นป้ายห้อง ไม่ใช่ mark ของ element) ตัวเลขเพดาน
จึงเป็น **ประมาณการเชิงอนุรักษ์** ไม่ใช่ค่าที่พิสูจน์แล้ว

### ✅ 2026-08-25 (ต่อ) — แก้ตัววัดแล้วจริง (ทางแก้ข้อ 1+2 จากหัวข้อบน)

มะขามสั่ง "ไปหาในเน็ตและแก้มา" → อิงมาตรฐาน KIE (KIEval, arXiv:2503.05488 — รายงาน exact
กับ relaxed แยกคอลัมน์ + normalize ก่อนเทียบ) แก้ `infer_house_t03.py` แล้ว **GT ไม่ถูกแตะ**:

- `norm_id()` — strip / ยุบช่องว่าง / casefold ก่อนเทียบ (กัน `"B1 "` ≠ `"B1"`)
- `score_ids()` — คืน 2 คอลัมน์: **recall รวม** (ทุก id, เทียบย้อนรอบเก่าได้) และ
  **recall_printed** (เฉพาะ id ที่เข้า `PRINTED_ID` = ชื่อที่พิมพ์บนแบบจริง — ตัวตัดสินโมเดล)
- `_summary.json` ได้ field ใหม่ `gt_printed`/`hit_printed`/`recall_printed` (ของเดิมคงอยู่ครบ)
- สรุปท้ายรันพิมพ์ทั้งสองคอลัมน์ + บรรทัดอธิบายวิธีอ่าน

**ยืนยันด้วย self-check กับ GT จริงของบ้าน 08** (ตัวให้คะแนนเป็นโค้ดล้วน ไม่ต้องใช้ GPU):
โมเดลสมบูรณ์แบบ → 100%/100% · โมเดลที่อ่านออกเฉพาะชื่อบนแบบ → recall รวม 12% แต่
printed 100% (สองคอลัมน์แยกโรคกันจริง) · โมเดลถูกหมดแต่มีช่องว่าง/ตัวพิมพ์ปน → 100%/100%
(ตัววัดเก่าให้ 0) — assert ครบทุกกรณี

**สำคัญตอนอ่านผลรอบหน้า:** คอลัมน์ recall รวมยังเทียบ t02/t03 เดิมได้ แต่ตัวเลขที่ใช้ตัดสินว่า
โมเดลอ่านแบบเป็นหรือไม่คือ **recall_printed** — section ของบ้าน 08 มี printed id แค่ 1 ตัว
งานส่วนใหญ่จะขึ้น "printed -" ซึ่งเป็นความจริงของข้อมูล ไม่ใช่บั๊ก

**พ่วง:** `MAX_PIXELS` ใน infer script มีคอมเมนต์เตือนแล้วว่า t03b เทรนที่ 7680 — ต้องแก้ตาม
ก่อนวัด t03b (ตอนนี้คง 5120 ไว้ให้ตรงกับ adapter t03 ตัวปัจจุบัน)

**ยังไม่ทำ (แตะ GT = รอมะขามอนุมัติ):** เปลี่ยน convention การตั้ง element_id ของ section
และ/หรือแยก section ออกเป็นหน้าตัดโครงสร้าง vs รายละเอียดสถาปัตย์ (ทางแก้ข้อ 3)

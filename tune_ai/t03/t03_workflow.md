...
## 🔧 บทเรียนใหม่ 2026-08-25 04:20 — เทรนจบ, inference ครั้งแรกพัง 2/2, แก้แล้วรันใหม่ด้วย xgrammar เต็ม

**เทรนจบสมบูรณ์:** 3 epochs (321 step), `train_loss: 0.2249`, peak VRAM 87.5GB, adapter เซฟ
ครบ (3.78GB) · **verify PASS** (sha256 ทีละไฟล์ 7 ไฟล์) · **อัป HF สำเร็จ** (`Sicilian44/t03`
commit `9530b9a`) — มีสำเนา 2 ที่แล้วก่อนแตะ destroy

**ยิงบ้าน 08 ครั้งแรก (ไม่มี xgrammar นอก plan_beam) — 2/2 งานแรก JSON เสียทั้งคู่**, คนละสาเหตุ:
1. **gridline** (925s): โครงสร้างพังจริง — ขาด `{` เปิด array element, field name เพี้ยน
   (`pox_M` แทน `pos_m`) — งานยากสุด+ตัวอย่างเทรนน้อยสุดในชุด (45 ตัวอย่าง) ยังไม่แก้ ต้องรอ
   op04 ขยายข้อมูล gridline รอบหน้า
2. **notes** (597s): `"confidence_score": 0.,` — เลขทศนิยมไม่มีหลักตามหลังจุด (JSON ไม่ยอมรับ)
   ตรวจพบว่ามาจาก field `warnings[]` **ไม่ใช่** data pollution (307 ไฟล์ทั่วชุดข้อมูลมี
   `warnings[]` พูดถึง "NORMALIZED_2026-08-02" จริง — เป็น metadata field ที่ตั้งใจให้มี ไม่ใช่บั๊ก)
   **ลึกลงไปอีกจุดในไฟล์เดียวกัน** เจอบั๊กที่ 2 คนละชนิด: `{item_no":` ขาด `"` เปิดหน้าชื่อ key

**ตัดสินแล้ว ✅ — เปลี่ยนไปใช้ `--grammar-all` (xgrammar บังคับทุก subtask) แทนไล่ patch
regex ทีละแบบ:** 2 บั๊กที่เจอเป็นคนละชนิดกันในไฟล์เดียวกัน แปลว่ามีบั๊กคลาสนี้อีกไม่รู้กี่แบบ
รอข้างหน้า — regex ไล่แก้ทีละแบบไม่มีวันจบ ขณะที่ xgrammar ป้องกันปัญหาทั้งคลาสที่ระดับไวยากรณ์
(พิสูจน์แล้ว 96.8% vs 57.9%, rule_of_tune ข้อ 13) เพิ่ม `re.sub(r"(\d)\.(?=[,}\]\s])", r"\1.0", t)`
ใน `strip_fence()` ไว้เป็นชั้นป้องกันสำรอง (defense in depth, ไม่ถอดออก) **ความเสี่ยงที่รับ:**
generate ช้าลงจาก constrained-decoding overhead, known limitation เดิมยังอยู่ (วนซ้ำในสตริง
คุมไม่ได้ แต่มี `PAGE_TIMEOUT_S=25min` กันไว้แล้ว) · kill process เดิม ลบผลค้าง (`ผล_t03/tuned`
ที่มีแค่ 2/33 งาน) → รันใหม่ทั้งชุดด้วย `--grammar-all` (เสียไปแค่ ~$0.4, 25 นาที)

## 📊 ผลจริง — บ้าน 08, t03 (tuned) 2026-08-25 07:40 ครบ 33/33 งาน

**JSON valid 32/33 (97%)** · **id-recall เฉลี่ย 10.1%** (จาก 22 งานที่ GT มี id ให้วัด)
เทียบ t02 บนบ้านเดียวกัน (JSON valid 96%, id-recall 3.5%) — **สูงกว่า t02 ในภาพรวม แต่ยังไม่ใช่
การเทียบที่ควบคุมตัวแปรจริง** (เหตุผลเดิม 6 ข้อใน t03_workflow — คนละ prompt/หน่วยงาน/decoding
stack) การเทียบที่ตัดตัวแปรกวนคือ **t03 vs base** ซึ่งกำลังรันอยู่ (ดูหัวข้อถัดไป)

| subtask | n | JSON valid | pred el / GT el | id-recall แต่ละงาน |
|---|---|---|---|---|
| gridline | 1 | 1/1 | 0/0 | — (ไม่มี element_id ใน gridline โดย schema) |
| notes | 4 | 4/4 | 12/0 | — (GT notes ไม่มี element_id — ใช้ sections[] แทน) |
| schedule | 5 | 5/5 | 11/32 | 43%, 29%, 0%, 0%, 0% |
| section | 13 | 13/13 | 34/23 | 0% ×8, บาง n/a |
| plan_footing | 4 | 4/4 | 3/6 | 0%, **100%**, 0%, 0% |
| **plan_beam** | **3** | 3/3 | **2/54** | **0%, 0%, 0%** |
| plan_slab | 2 | 1/2 | 1/6 | 0%, 50% |

**สิ่งที่ต้องพูดตรงๆ:** JSON valid ดีขึ้นชัดเจน (grammar ทำงานจริง) และ schedule/plan_footing/
plan_slab เริ่มมี recall เป็นบวกที่ t02 ไม่เคยทำได้เลยแม้แต่หน้าเดียว **แต่ plan_beam — subtask
ที่แบกความหวังทั้งรอบ — ยังคง 0% ทั้ง 3 งาน** งานหนึ่ง (หน้า 20 เดียวกับที่ t02 พังตอนบ่าย) โมเดล
เขียนแค่ field metadata แล้วปิด JSON โดยไม่มี `elements[]` เลย และ metadata ที่เขียนมาก็ชี้ไป
บ้านอื่น (`source_image` ชี้บ้าน 22) — เหมือนจำ boilerplate จากตัวอย่างเทรนที่คล้ายกันมาแปะ
แทนที่จะอ่านจากภาพจริง — **plan_beam ยังไม่ใช่ชัยชนะที่ตั้งใจไว้ตอนออกแบบ pass_design.csv**

**บทเรียนระหว่างรัน (สคริปต์ ไม่ใช่โมเดล):** ครั้งแรกไม่มี xgrammar นอก plan_beam → 2/2 งานแรก
JSON เสีย (บั๊กคนละชนิด: gridline โครงสร้างพังจริง, notes เลขทศนิยมไม่ครบ+ขาด quote หน้า key)
→ ตัดสินใช้ `--grammar-all` บังคับทุก subtask แทนไล่ patch regex → รันใหม่ทั้งชุด (เสีย ~$0.4)

## 🔬 กำลังรัน — Qwen3.6 base (untuned) บนบ้าน 08 ชุดงานเดียวกันเป๊ะ (2026-08-25 07:40 เริ่ม)

**ตัดสินแล้ว ✅:** รันเต็ม 33 งาน ไม่ตัดขอบเขต — บ้านเดียวกัน prompt เดียวกัน ตัววัดเดียวกัน
ต่างกันแค่ adapter = การเทียบที่ตัดตัวแปรกวนทั้ง 6 ข้อที่ทำให้เทียบกับ t02 ตรงๆ ไม่ได้ ถือเป็น
แกนกลางของค่ำคืนนี้ ตัดครึ่งจะทำลายจุดแข็งของการทดลอง เครดิตเหลือ $22.69 ยังพอเหลือเฟือ
คาดว่าใช้เวลาใกล้เคียงรอบ tuned (~3 ชม.) — จบ ~10:40
...
| 2e | **git commit ผลลัพธ์รอบนี้** (มะขามสั่งเพิ่ม 2026-08-25 — ".5") | `git add -A && git commit` ใน repo `Training` — ตัวเลขวัดผล/ภาพเปรียบเทียบ/overlay/model-card ที่อัปเดตหลังข้อ 2a-2d ต้องอยู่ใน commit ก่อน shutdown ไม่ใช่ค้างเป็น working tree เฉยๆ (บทเรียนเดิมคืนนี้: commit ทันทีที่มีอะไรจบ ไม่ปล่อยค้าง) | ⬜ |

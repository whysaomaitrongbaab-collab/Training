...
def analyze(gray, tpls_footing=(), tpls_column=(),
    ...
    # ⚠️ แก้ 2026-09-01: เดิมเช็ค f[0],f[1] = **มุมบนซ้าย** ทั้งที่คอมเมนต์ข้างบนเขียนว่า
    # "จุดกึ่งกลาง" — โค้ดขัดกับเจตนาตัวเอง ผลคือกล่องฐานรากที่มุมไปแตะแถบคานก็โดนตัด
    # ทั้งที่ตัวฐานรากไม่ได้อยู่บนคาน · เรื่องนี้โผล่กับ **ผังรวม** (footing+คาน+เสาหน้าเดียว
    # เช่น บ้านครอบครัวไทยเป็นสุข2 หน้า12 "แปลนฐานราก, ตอม่อ, เสา คาน พื้น") ซึ่งหักล้าง
    # สมมติฐานเดิมที่ว่า "ผังฐานรากจริงไม่มีคานให้ detect" — บ้านนั้นตัดเหลือ 1 จาก 17
    # sweep 15 บ้านในคลังก่อนแก้: 14 บ้านผลเท่าเดิมเป๊ะ (กฎนี้แทบไม่ทำงานบนผังฐานรากจริง),
    # ต่างแค่บ้าน 06 (34→31, GT 33 — ยังอยู่ในเกณฑ์ ✅), ไม่มีบ้านไหนกลายเป็นขยะเกิน GT
    ...
    out["footing"] = [f for f in out["footing"]
                      if not any(bx - M <= f[0] + f[2] // 2 <= bx + bw + M
                                 and by - M <= f[1] + f[3] // 2 <= by + bh + M
